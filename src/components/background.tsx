...
export function Background() {
  ...
  useEffect(() => {
    ...
    const config = {
      ...
      opacity: 0.15, // Visible but not distracting
    };
    ...
    const draw = () => {
      ...
      for (let i = 0; i < config.lineCount; i++) {
        ...
        for (let x = 0; x <= width; x += 10) {
          ...
          const noise2 = Math.cos(x * config.frequency * 0.5 - time * 0.3 + i * 0.1);
          
          // Combine waves
          // Vary amplitude based on position to create "breathing" effect
          const currentAmp = config.amplitude * (1 + Math.sin(time * 0.5 + i * 0.1) * 0.3);
